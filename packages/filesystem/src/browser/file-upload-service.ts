        ...
        const callback = async (response: any) => {
            if (response.uri) {
                ...
                if (!folderMap.get(new URI(response.uri).parent.toString())) {
                    this.remoteFileSystemProvider.delCache(false, new URI(response.uri).parent.parent);
                    this.remoteFileSystemProvider.emitter.fire([{ type: FileChangeType.ADDED, resource: new URI(response.uri).parent }]);
                }
                this.remoteFileSystemProvider.delCache(false, new URI(response.uri).parent);
